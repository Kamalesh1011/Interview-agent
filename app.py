...
# Optional: FAISS RAG engine (comment out if not installed)
try:
    from backend.rag_engine import add_interview_to_faiss
    FAISS_AVAILABLE = True
except ImportError:
    print("⚠️ FAISS not available - RAG features disabled")
    FAISS_AVAILABLE = False
    def add_interview_to_faiss(*args, **kwargs):
        pass  # Dummy function
...
llm = LLM(
    ...
    api_key="AIzaSyAS7ZKuAXsk1-3F2ZX8RpRlBVs8dAS63Qk"
)
...
def create_fallback_evaluation(response, quality):
    """STRICT fallback scoring (NEVER inflate above 4)"""

    # STRICT non-inflated scoring mapping
    quality_scores = {
        "empty": 0,
        "minimal": 1,
        "no_knowledge": 1,
    ...
    # default strict fallback = 2 (NOT 5)
    base_score = quality_scores.get(quality, 2)
    base_score = min(base_score, 4)

    evidence_map = {
        "empty": "No response provided",
        "minimal": "Response too brief to evaluate",
        "no_knowledge": "Candidate admitted lack of knowledge",
        "very_brief": "Response lacks detail",
        "brief": "Limited elaboration provided",
        "confused": "Shows confusion with the question",
        "uncertain": "Candidate unsure about answer",
        "normal": "Basic response without depth",
        "verbose": "Detailed response but not fully evaluated"
    }

    evidence = evidence_map.get(quality, "Auto fallback evaluation")
    ...
    return {
        "scores": {
            ...
            "practical_experience": {
                "score": base_score,
                "evidence": "No practical example detected"
            },
            "relevance_to_question": {"score": base_score, "evidence": evidence},
        },
        "overall_score": base_score,
        "response_type": quality,
        "what_they_demonstrated": [] if base_score <= 2 else ["Basic understanding"],
        "what_was_missing": [
            "Detailed explanation",
            "Technical depth",
            "Concrete examples"
        ],
        "needs_follow_up": base_score <= 2,
        "follow_up_reason": f"Low-quality response ({quality})",
        "suggested_approach": "Ask follow-up question to verify understanding"
    }
...
def create_fallback_report(interview_data, jd_match, evaluations):
    """Strict fallback report — NO score inflation"""

    avg_score = 3   # strict default
    
    if evaluations:
        scores = [e.get("overall_score", 2) for e in evaluations 
                  if isinstance(e.get("overall_score"), (int, float))]
        
        if scores:
            avg_score = sum(scores) / len(scores)

    avg_score = min(avg_score, 4)   # hard cap
    ...
    return {
        "executive_summary": {
            ...
            "confidence": "Medium",
            "one_line": "Fallback evaluation applied.",
            "key_evidence": "Fallback mode triggered."
        },
        "skills_assessment": {
            "verified_skills": [],
            "claimed_but_unverified": [],
            "concerning_gaps": ["Insufficient detail in responses"]
        },
        "jd_alignment": {
            "match_percentage": jd_match.get("overall_match_percentage", 40),
            "must_haves_met": [],
            "must_haves_missing": []
        },
        "behavioral_observations": {
            "communication_style": "Basic",
            "response_patterns": "Short responses",
            "red_flags": ["Lack of detail"],
            "green_flags": []
        },
        "detailed_recommendation": {
            "hire_decision": f"Based on strict fallback score {round(avg_score,1)}",
            "role_fit": "Uncertain",
            "growth_potential": "Needs evaluation",
            "onboarding_needs": [],
            "interview_confidence": "Medium"
        },
        "next_steps": "Recommend second-round interview"
    }
...
@app.route("/agentic-simulation")
def agentic_simulation():
    return render_template('agentic-simulation.html')
...
# ============================
    malpractice_reason = interview.get("malpractice_reason")

    if malpractice_reason:
        print("⚠️ Malpractice detected — overriding score to 0")

        # Force all scoring to zero
        final_report["executive_summary"]["overall_score"] = 0
        final_report["executive_summary"]["recommendation"] = "Interview invalidated due to malpractice."

        # Optional: Add tag
        final_report["malpractice_flag"] = True
        final_report["malpractice_reason"] = malpractice_reason

        # Also override internal interview scoring
        interview["overall_score"] = 0
        if request.json.get("terminated"):
            interview["malpractice_reason"] = request.json.get("reason")
